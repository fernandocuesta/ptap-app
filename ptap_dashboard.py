...
import os
import streamlit as st
st.write("App iniciada")  # Agrega esto justo después de los imports
...
# KPIs y visualización por locación
...
# Historial con filtros y eliminación
    ...
    for i, row in df.iterrows():
        with st.expander(f"📌 {row['Fecha'].date()} - {row['Hora']} - {row['Locación']}"):
            ...
            color_ph = "🟢" if 6.5 <= row['pH'] <= 8.5 else "🔴"
            color_tur = "🟢" if row['Turbidez (NTU)'] <= 5 else "🔴"
            color_clo = "🟢" if 0.2 <= row['Cloro Residual (mg/L)'] <= 1.5 else "🔴"
            st.write(f"{color_ph} **pH:** {row['pH']} | {color_tur} **Turbidez:** {row['Turbidez (NTU)']} | {color_clo} **Cloro:** {row['Cloro Residual (mg/L)']}")
    ...
    if st.button("📥 Generar PDF del resumen actual"):
        ...
        for i, row in df.tail(10).iterrows():
            ...
            if isinstance(row['Foto'], str) and row['Foto'].strip():
                img_path = os.path.join(FOTOS_DIR, row['Foto'])
                if os.path.exists(img_path):
                    try:
                        img = Image.open(img_path)
                        img.thumbnail((100, 100))
                        img.save("temp_thumb.jpg")
                        pdf.image("temp_thumb.jpg", w=30)
                    except:
                        pdf.cell(0, 10, txt="[Imagen no compatible]", ln=True)
