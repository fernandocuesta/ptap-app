...
if menu == "➕ Ingreso de muestra":
    ...
    if df.empty:
        st.info("No hay registros disponibles para análisis.")
    else:
    ...
    if df.empty:
        ...
        with colf3:
            loc_fil = st.selectbox("Locación", ["Todas"] + sorted(df["Locación"].dropna().unique()))
        df = df[(df["Fecha"] >= pd.to_datetime(fecha_ini)) & (df["Fecha"] <= pd.to_datetime(fecha_fin))]
        if loc_fil != "Todas":
            df = df[df["Locación"] == loc_fil]
        for i, row in df.iterrows():
            with st.expander(f"📌 {row.get('Fecha', '')} - {row.get('Hora', '')} - {row.get('Locación', '')}"):
                st.write(f"👷 **Técnico:** {row.get('Técnico', 'No registrado')}")
                st.write(f"pH: {row.get('pH', 'N/A')} | Turbidez: {row.get('Turbidez (NTU)', 'N/A')} | Cloro: {row.get('Cloro Residual (mg/L)', 'N/A')}")
                st.write(f"📝 **Observaciones:** {row.get('Observaciones', 'Sin observaciones')}")

elif menu == "📤 Exportar PDF":
    st.title("📤 Exportar registros")
    df = st.session_state.data.copy()
    if not df.empty:
        st.download_button("⬇️ Descargar Excel", data=df.to_csv(index=False), file_name="ptap_mediciones.csv")
    else:
